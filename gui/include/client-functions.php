...
function get_domain_dns_serial($Input) {
...
function set_domain_dns_serial($Input) {
	...
	$sql_query = "
			UPDATE
				`domain`
			SET
				`domain_dns_serial` = :serial
			WHERE
				".$Where;
	
	$Serial = get_domain_dns_serial($Input);
	$Serial = explode("-", $Serial);
	
	if ($Serial[0]==date(Y) && $Serial[1]==date("m") && $Serial[2]==date("d")) {
		$New_Serial = $Serial[0]."-".$Serial[1]."-".$Serial[2]."-".($Serial[3]+1);
	}
	else {
