	...
	if (ptr != buff) {
		...
		if (fork() == 0) {
			...
			if (readlink (fname1, fname2, sizeof (fname2)) > 0) {
				strncpy(daemon_path, fname2, strlen(fname2)-strlen("daemon/easyscp_daemon"));
				strcat(daemon_path, "engine/easyscp-rqst-mngr");
				...
				execl(daemon_path, "easyscp-rqst-mngr" ,(char*)NULL);
			}
			...
			*/
			...
			sprintf(
				qcommand,
				"%s 1>%s/%s.%ld 2>%s/%s.%ld",
				"/usr/local/www/easyscp/engine/easyscp-rqst-mngr",
				LOG_DIR,
				STDOUT_LOG,
				(long int) tim,
				LOG_DIR,
