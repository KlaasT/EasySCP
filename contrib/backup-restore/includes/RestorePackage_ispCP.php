 ...
 * @version 	SVN: $Id: RestorePackage_ispCP.php 3721 2010-12-20 17:53:14Z ShadowJumper $
 *
 * @license
...
class RestorePackage_ispCP extends BaseController
{
	...
	public function __construct($domain_name, $password, $option_ip, $option_reseller, $log_level)
	{
		...
		$this->db = EasySCP_Database::getInstance();
		$this->ip = $option_ip;
		...
		$this->target_path = EasySCP_VIRTUAL_PATH.'/'.$this->domain_name;
	...
	protected function getDefaultIPID()
	{
		...
		if ($rs && !$rs->EOF) {
			...
			$this->logMessage('No IPs found!?', EasySCP_LOG_ERROR);
		}
	...
	protected function getDefaultResellerID()
	{
		...
		if ($rs && !$rs->EOF) {
			...
			$this->logMessage('No resellers found!?', EasySCP_LOG_ERROR);
		}
	...
	protected function getResellerID($reseller)
	{
		...
		if ($rs && !$rs->EOF) {
			...
			$this->logMessage('Reseller not found: '.$reseller, EasySCP_LOG_ERROR);
		}
	...
	protected function getResellerUserProps()
	{
		...
		if ($rs && !$rs->EOF) {
			...
			$this->logMessage('Reseller user properties not found: '.$this->reseller_id, EasySCP_LOG_ERROR);
			$this->reseller_prop_lang = 'lang_EnglishBritain';
			$this->reseller_prop_layout = 'default';
	...
	protected function getIPID($ip)
	{
		...
		if ($rs && !$rs->EOF) {
			...
			$this->logMessage('IP not found: '.$ip, EasySCP_LOG_ERROR);
		}
	...
	protected function initRestore()
	{
		...
		if (!file_exists($this->gpg_archive)) {
			$this->logMessage('Domain backup package file not found: '.$this->gpg_archive, EasySCP_LOG_ERROR);
		} else {
			// IP detection
			if ($this->ip === false) {
	...
	protected function unpackDomainPackage()
	{
		...
		if (file_exists($this->archive)) {
			...
			$cmd = 'tar xzf '.$this->archive.' -C '.EasySCP_VIRTUAL_PATH;
			$this->shellExecute($cmd, $a);

			if (file_exists($this->target_path.'/tmp/config.ser')) {
				$result = true;
			} else {
				$this->logMessage('File not found: '.$this->target_path.'/tmp/config.ser', EasySCP_LOG_ERROR);
			}

			$result = true;
		} else {
			$this->logMessage('File not found: '.$this->archive.' - incorrect password?', EasySCP_LOG_ERROR);
		}
	...
	protected function getConfigData()
	{
		...
		$this->logMessage('getConfigData', EasySCP_LOG_INFO);
		...
		if ($fp) {
			...
			if (is_array($this->configurationData) && count($this->configurationData) > 0) {
				...
				$this->logMessage('configurationData OK', EasySCP_LOG_INFO);

				$result = true;
			} else {
				$this->logMessage('Broken file '.$config_file, EasySCP_LOG_ERROR);
			}

		} else {
			$this->logMessage('Could not open file '.$config_file, EasySCP_LOG_ERROR);
		}
	...
	protected function createDatabases()
	{
		foreach ($this->configurationData['db'] as $db) {
			$this->logMessage('createDatabases: '.$db['sqld_name'], EasySCP_LOG_INFO);
	...
	private function importMySQLDatabase($dbname)
	{
		...
		$cmd = 'mysql --user '.EasySCP_Config::get('DB_USER').' --password='.EasySCP_Config::get('DB_PASS').
			   ' '.$dbname.
			   ' <'.$filename;
		// TODO: Error handling
		$a = array();
	...
	{
		$this->logMessage('createDatabaseUser: '.$db_user, EasySCP_LOG_INFO);
	...
	protected function createDomain()
	{
		...
		$this->logMessage('createDomain', EasySCP_LOG_INFO);
		...
		if (!$this->db->execute($query, $params)) {
			$this->logMessage('Cannot insert admin database entry!', EasySCP_LOG_ERROR);
			return false;
		}
		$domain_admin_id = $this->db->insertId();
		$this->logMessage('Domain Admin ID: '.$domain_admin_id, EasySCP_LOG_DEBUG);
		...
		if (!$this->db->execute($query, $params)) {
			$this->logMessage('Cannot insert domain database entry!', EasySCP_LOG_ERROR);
			return false;
		}
		...
		do {
			...
			$tries++;
			if ($tries > 60) {
				$this->logMessage('Error executing domain creation request!', EasySCP_LOG_ERROR);
				$result = false;
				break;
			}
		} while (!$daemon_ready);
	...
	protected function createDomainAliases()
	{
		...
		foreach ($this->configurationData['alias'] as $alias) {
			$this->logMessage('Create alias '.$alias['alias_name'], EasySCP_LOG_INFO);
			...
			$this->logMessage(' old alias id='.$alias['alias_id'].' new alias id='.$alias_id, EasySCP_LOG_DEBUG);
	...
	protected function createSubDomains()
	{
		...
		foreach ($this->configurationData['subdomain'] as $subdomain) {
			$this->logMessage('Create subdomain '.$subdomain['subdomain_name'], EasySCP_LOG_INFO);
	...
	{
		$this->logMessage('createEMailAccount: '.$email['mail_acc'], EasySCP_LOG_INFO);
		...
		if (isset($email['sub_id']) && !empty($email['sub_id']) && isset($email['mail_type'])) {
			...
			if ($n !== false) {
				...
				if (!isset($this->subdomain_ids[$email['sub_id']])) {
					$this->logMessage('domain subdomain id not found: '.$email['sub_id'], EasySCP_LOG_INFO);
					return;
				} else {
				...
				if (!isset($this->domain_alias_ids[$email['sub_id']])) {
					$this->logMessage('domain alias id not found: '.$email['sub_id'], EasySCP_LOG_INFO);
					return;
				} else {
	...
	protected function createFTPAccounts()
	{
		...
		foreach ($this->configurationData['ftp'] as $ftp) {
			...
			$this->logMessage('createFTPAccounts: '.$ftp['userid'], EasySCP_LOG_INFO);
	...
	protected function createWebUsers()
	{
		...
		foreach ($this->configurationData['webuser'] as $webuser) {
			...
			$this->logMessage('createWebUsers: '.$webuser['uname'], EasySCP_LOG_INFO);
	...
	protected function createWebGroups()
	{
		...
		foreach ($this->configurationData['webgroup'] as $webgroup) {
			...
			$this->logMessage('createWebGroups: '.$webgroup['ugroup'], EasySCP_LOG_INFO);
	...
	protected function createWebAccess()
	{
		foreach ($this->configurationData['webaccess'] as $webaccess) {
			...
			$this->logMessage('createWebAccess: '.$webaccess['path'], EasySCP_LOG_INFO);
			...
			if (!empty($webaccess['user_id'])) {
				if (isset($this->webuser_ids[$webaccess['user_id']])) {
					...
					$this->logMessage('createWebAccess: missing user_id', EasySCP_LOG_DEBUG);
				}
			}
			if (!empty($webaccess['group_id'])) {
				if (isset($this->webgroup_ids[$webaccess['group_id']])) {
					...
					$this->logMessage('createWebAccess: missing group_id', EasySCP_LOG_DEBUG);
				}
			}
	...
	protected function createDNSEntries()
	{
		...
		$this->logMessage('createDNSEntries: currently not supported', EasySCP_LOG_DEBUG);
	}
