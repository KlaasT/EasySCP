 ...
 * @version 	SVN: $Id: BackupPackage.php 3095 2010-07-16 15:50:36Z nuxwin $
 *
 * @license
...
abstract class BackupPackage extends BaseController
{
	...
	public function __construct($domain_name, $password, $log_level)
	{
		...
		$this->backup_temp_path = EasySCP_VIRTUAL_PATH.'/'.$this->domain_name.'/tmp';
	...
	private function writeDomainConfig()
	{
		...
		if ($fp) {
			...
			$result = false;
			$this->logMessage('Could not create file '.$this->config_file, EasySCP_LOG_ERROR);
		}
	...
	private function dumpMySQLDatabase($dbname)
	{
		...
		$cmd = 'mysqldump --user '.EasySCP_Config::get('DB_USER')
				.' --password='.EasySCP_Config::get('DB_PASS')
				.' '.$dbname
				.' >'.$filename;
		// TODO: Error handling
		$a = array();
	...
	private function createDomainPackage()
	{
		...
		$cmd = 'tar czf '.$filename.
				' -C '.EasySCP_VIRTUAL_PATH.' '.$this->domain_name.
