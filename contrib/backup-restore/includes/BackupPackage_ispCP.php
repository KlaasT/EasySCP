 ...
 * @version 	SVN: $Id: BackupPackage_ispCP.php 3059 2010-07-02 01:52:27Z nuxwin $
 *
 * @license
...
class BackupPackage_ispCP extends BackupPackage implements iBackupPackage
{
	...
	protected function initDomain()
	{
		...
		if (!file_exists(EasySCP_VIRTUAL_PATH.'/'.$this->domain_name)) {
			$this->logMessage('Domain not found in '.EasySCP_VIRTUAL_PATH.'/'.$this->domain_name, EasySCP_LOG_ERROR);
		} else {
			$test = $this->getDomainID($this->domain_name);
			if ($test != -1) {
				$result = true;
			} else {
				$this->logMessage('Domain not in database: '.$this->domain_name, EasySCP_LOG_ERROR);
			}
		}
	...
	public function getDomainConfig()
	{
		...
		if ($rs && $rs->recordCount() > 0) {
			...
			$this->logMessage('Error reading domain configuration from database!', EasySCP_LOG_ERROR);
		}
